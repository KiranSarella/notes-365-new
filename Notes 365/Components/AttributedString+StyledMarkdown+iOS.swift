...
extension AttributedString {
    ...
    init(styledMarkdown markdownString: String, fontSize: CGFloat = UIFont.preferredFont(forTextStyle: .body).pointSize) throws {
        ...
        s.foregroundColor = UIColor.label
        ...
        for inlineIntent in inlineIntents {
            ...
            switch inlineIntent {
            case .emphasized:
                targetAttributeContainer.font = UIFont.systemFont(ofSize: fontSize) // .italicSystemFont(ofSize: fontSize)
                
                targetAttributeContainer.foregroundColor = .green
                
            case .stronglyEmphasized:
                targetAttributeContainer.font = .systemFont(ofSize: fontSize, weight: .bold)
                targetAttributeContainer.foregroundColor = UIColor.red
            case .code:
                targetAttributeContainer.font = .monospacedSystemFont(ofSize: fontSize, weight: .regular)
                targetAttributeContainer.backgroundColor = UIColor.systemOrange
            case .strikethrough:
        ...
        for (intentBlock, intentRange) in s.runs[AttributeScopes.FoundationAttributes.PresentationIntentAttribute.self].reversed() {
            ...
            case .headline(let level):
                ...
                case 1:
                    ...
                    s[intentRange].foregroundColor = UIColor.systemTeal
                case 2:
                    s[intentRange].font = .systemFont(ofSize: 26, weight: .heavy)
                    s[intentRange].foregroundColor = UIColor.systemCyan
                case 3:
                    s[intentRange].font = .systemFont(ofSize: 24, weight: .heavy)
                    s[intentRange].foregroundColor = UIColor.systemMint
                case 4:
                    s[intentRange].font = .systemFont(ofSize: 22, weight: .heavy)
                    s[intentRange].foregroundColor = UIColor.systemBrown
                case 5:
                    s[intentRange].font = .systemFont(ofSize: 20, weight: .heavy)
                    s[intentRange].foregroundColor = UIColor.systemIndigo
                default:
                    // TODO: Handle H4 to H6
                    s[intentRange].font = .systemFont(ofSize: 15, weight: .heavy)
                    s[intentRange].foregroundColor = UIColor.systemPurple
                }
            case .paragraph:
                break
            case .unorderedListElement:
            ...
            case .blockquote:
                ...
                s[intentRange].foregroundColor = UIColor.secondaryLabel
            case .code:
