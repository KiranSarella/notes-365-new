...
extension AttributedString {
    ...
    init(styledMarkdown markdownString: String, fontSize: CGFloat = NSFont.preferredFont(forTextStyle: .body).pointSize) throws {
...
//        print(s)
        ...
        s.foregroundColor = NSColor.textColor
        ...
        for inlineIntent in inlineIntents {
            ...
            switch inlineIntent {
            case .emphasized:
                targetAttributeContainer.font = NSFont.systemFont(ofSize: fontSize) // .italicSystemFont(ofSize: fontSize)
                
                targetAttributeContainer.foregroundColor = .green
                
            case .stronglyEmphasized:
                targetAttributeContainer.font = .systemFont(ofSize: fontSize, weight: .bold)
                targetAttributeContainer.foregroundColor = NSColor.red
            case .code:
                targetAttributeContainer.font = .monospacedSystemFont(ofSize: fontSize, weight: .regular)
                targetAttributeContainer.backgroundColor = NSColor.systemOrange
            case .strikethrough:
...
//                assertionFailure(intentBlock.debugDescription)
            ...
            case .headline(let level):
                ...
                case 1:
                    ...
                    s[intentRange].foregroundColor = NSColor.systemTeal
                case 2:
                    s[intentRange].font = .systemFont(ofSize: 26, weight: .heavy)
                    s[intentRange].foregroundColor = NSColor.systemCyan
                case 3:
                    s[intentRange].font = .systemFont(ofSize: 24, weight: .heavy)
                    s[intentRange].foregroundColor = NSColor.systemMint
                case 4:
                    s[intentRange].font = .systemFont(ofSize: 22, weight: .heavy)
                    s[intentRange].foregroundColor = NSColor.systemBrown
                case 5:
                    s[intentRange].font = .systemFont(ofSize: 20, weight: .heavy)
                    s[intentRange].foregroundColor = NSColor.systemIndigo
                default:
                    // TODO: Handle H4 to H6
                    s[intentRange].font = .systemFont(ofSize: 15, weight: .heavy)
                    s[intentRange].foregroundColor = NSColor.systemPurple
                }
            case .paragraph:
                break
            case .unorderedListElement:
            ...
            case .blockquote:
                ...
                s[intentRange].foregroundColor = NSColor.secondaryLabelColor
            case .code:
