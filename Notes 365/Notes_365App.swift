...
//        return try! ModelContainer(for: Notebook.self, NotebookContent.self, configurations: conf)
        do {
            let container = try ModelContainer(for: NotebookData.self, NotebookContent.self, TodayVersion.self, TimelineContent.self,
                                               configurations: conf)
            return container
        } catch {
            print("errror: \(error)")
            // fallback to local container
            return try! ModelContainer(for: NotebookData.self, NotebookContent.self, TodayVersion.self, TimelineContent.self)
        }
    }()
